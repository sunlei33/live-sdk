...
- **交互**：
  ...
  4. 在直播中（内核 `isLive === true`；注意此时 `duration` **可能是有限值**）调 `seek()`。
- **预期**：
  ...
  4. `seek` 在直播中 `applied === false`（判据见 US-53：由内核 `isLive` 回答，不从 `duration` 反推）；在有限时长（点播/重播）下 `applied === true` 且定位生效。
...
## 十五、破坏性变更（0.6.0 · 0.7.0）

> 本节的两条都是**移除**。判据相同：被移除的东西要么已被更好的机制取代，要么不属于 SDK 的核心职责。
> 升级前请对照自查 —— 两者都不会在编译期以外的地方报错（JS 接入方尤其要看）。
>
> ⚠️ **两条属不同版本**：**US-49（`sniffer` 命名空间）是 0.7.0** —— 该命名空间在 0.6.0 里**仍导出**；
> **US-50（内置 SentryReporter）是 0.6.0**，早已生效。判据是拆开两版的 npm tarball 按符号 grep
> （方法与逐项证据见 `implementation.md` §8.23）。

### US-49 能力探测归位：`sniffer` 命名空间移除（**0.7.0**；`isIOS` 等 UA 嗅探那一批更早已删）
...
### US-50 上报适配器不再内置（第三方绑定移出核心）（**0.6.0**）
...
## 十六、平台契约与自定义平台（P0/P1 解耦改造，0.7.0）
...
### US-53 直播判据由内核回答（`Kernel.isLive`，0.7.0）

- **目标**：`seek()` 的 noop 判定、以及「近尾判 `ended`」都必须基于**真实的直播状态**，
  而不是 `duration` 是否有限 —— 后者在 MSE 路径下**无法区分直播与点播**。
- **配置**：默认。自定义平台（自己的 `Kernel` 实现）可选实现 `isLive?()`。
- **背景（判据为什么换）**：

  | 内核 | 直播流下 `duration` | 成因 |
  |---|---|---|
  | `NativeKernel`（原生 HLS） | `Infinity` | 符合直觉 |
  | `HlsKernel`（MSE） | **有限值** = playlist edge（随滑窗递增） | hls.js 默认 `liveDurationInfinity: false` |

  即「`duration` 有限」无法推出「这是点播」。hls.js 自己的 `details.live`
  （语义 = playlist 未出现 `#EXT-X-ENDLIST`）才是权威判据，且随 playlist 重载自动翻转。
...
### US-54 运行期消息：稳定编号 + 可配置语言（0.7.0）

- **目标**：错误与日志既能被**稳定引用**（编号），又能按需选语言（默认英文）—— 海外接入方不必猜中文，
  国内排查者一行配置切中文；而「这是哪一条消息」的引用方式不随语言与版本漂移。
- **配置**：`locale`（`'en'` 默认 | `'zh'`）。无其它开关。
- **交互**：
  1. 触发各类错误：主 playlist 404、解码失败（`MediaError.code = 3`）、`play()` 被自动播放策略拦截、加载超时。
  2. 读 `player.on('error')` 的 `e.message`。
  3. 看控制台日志（`logger.*` / `console.*`）。
  4. 看 `getFeatureStatus()` 各项的 `detail`。
  5. 再以 `createPlayer({ locale: 'zh' })` 建实例，重复 2–4 步。
- **预期**：
  1. 消息形如 `[LV-xxxx] <文案>`：**编号恒定，文案随 `locale` 换语言**。
  2. 默认（不传 `locale`）为**英文**；传 `'zh'` 为中文。
  3. 编号唯一且形如 `LV-<分区><序号>`（分区见 `MSG` 常量：1xxx 命令交互 / 2xxx 内核媒体 /
     3xxx 网络重连 / 4xxx 配置接入 / 5xxx 插件 / 6xxx 能力对齐 / 9xxx 兜底）。
  4. 机器可读字段（`code` / `domain` / `Events.*` / `COMMAND_NAMES`）**不受语言影响**，恒为英文标识。
  5. `Events.RETRY` 载荷与 `ReportRecord.data.message` 同样带编号、同样随 `locale` 变化。
  6. ⚠️ 文档明确要求**不要按 `message` 文本做分支或匹配** —— 要锚就锚 `[LV-xxxx]` 编号，或用 `code` / `domain`。
     （message 文本至今已变过三次：中文 → 「中文 / English」→ 单语编号化。）
  7. **文案表是穷尽映射**：登记了编号却没补某种语言的文案 → **编译失败**，不会拖到运行时。
  8. **UI 控件文案用的是同一套机制，只是取用入口不同**（见 US-55）：同一个 `locale`、同一张文案表，
     但走 `uiText()`（纯文案），**不带 `[LV-xxxx]` 编号** —— tooltip 面向终端用户，编号只是噪声。
  9. `locale` 语义是**全局**的（同 `setLogLevel`）：同一页面多实例共用最后一次设置；
     需要按实例区分语言时，消费与语言无关的 `code` / `domain` / `MSG`。**UI 也在这个语义内**。
- **验收方式**：单测（`test/i18n.test.ts` 锁定编号格式 / 唯一性 / 语言切换 / 插值 / 文案表完整性 /
  `uiText` 与 `t` 的唯一差别是编号前缀 / `LV-7xxx` 分区；`test/features.test.ts` 锁定 `detail` 的编号文本；
  `test/player.test.ts` 锁定 `locale` 生效与编号前缀）
  + 冒烟（`verify/smoke.mjs`：告警类断言一律**锚定 `[LV-xxxx]` 编号**而非文案，跑 `dist/` 产物）。

### US-55 UI 控件文案：继承 `locale`、不带编号（0.7.0）

- **目标**：内置控件上的字（悬停提示、无障碍名、下拉项）与日志共用同一个语言设置 ——
  业务传一次 `locale`，界面与日志同时切换；同时**不把 `[LV-xxxx]` 编号带到用户可见的位置**。
- **配置**：复用 US-54 的 `locale`（`'en'` 默认 | `'zh'`）。**不新增任何开关** ——
  UI 与日志是同一类「面向人的文本」，不该有两套语言设置。
- **交互**：
  1. `createPlayer({ container, locale: 'zh' })` + `mountDefaultUI(player)` 起播。
  2. 悬停全屏按钮，读其 `title`；再读 `aria-label`。
  3. 读音量滑块的 `aria-label`。
  4. 展开清晰度下拉，看首项文本。
  5. 进入全屏，再读全屏按钮的 `title`。（读屏用户听到的也是这个）
  6. 起播后**运行中**调 `setLocale('en')` —— 不做任何播放操作。
  7. `unmount()`（或 `player.destroy()`）之后再调 `setLocale('zh')`。
- **预期**：
  1. 默认（en）：全屏 `Fullscreen`、播放 `Play`、静音 `Mute`、滑块 `Volume`、下拉首项 `Auto`。
  2. `locale: 'zh'`：分别为 `全屏` / `播放` / `静音` / `音量` / `自动`。
  3. **这些文案里没有 `[LV-xxxx]` 编号**。编号仍然存在（`LV-7xxx` 分区），但只经 `t()` 出现在
     日志 / 错误 / 上报通道 —— 那里的读者是开发，需要编号来引用与告警锚定。
  4. 文案是**动作式**（写「点了会怎样」，而不是描述当前状态）：全屏态下提示 `退出全屏`；
     已静音时提示 `取消静音` —— 与图标语义一致，用户不必反推。
  5. **图标按钮必须有可访问名**：`▶` / `⏸` / `🔊` / `🔇` 对读屏软件没有语义（读出的是空白或符号名），
     因此每个按钮都带 `title` + `aria-label`（两者同文案）。音量滑块只给 `aria-label`
     （拖动时频繁弹提示是噪声）。
  6. 第 6 步：已挂载控件**立即**重绘，不需要任何播放操作。这是必需的设计 ——
     `player.subscribe` 只在**状态变化**时触发，而切语言不改状态，光靠它会出现
     「语言变了、tooltip 没变」且可能长期不刷新。实现是切语言时通知订阅者，控件用**最近状态快照**重绘。
  7. 第 7 步：已卸载的控件**不再被触碰**（监听器随 `unmount` 一并解绑，不泄漏）。
...
### US-56 运行期注册的插件也能拿到 `ready`（0.7.0）

- **目标**：`registerPlugin` 在内核**已就绪之后**被调用时，插件仍能收到一次 `ready()` ——
  否则「页面运行到一半才动态挂上来的插件」永远等不到就绪回调，只能自己在 `init` 里瞎猜时机。
- **配置**：无（`preset` / `ignores` 照常）。
- **交互**：
  1. 在 `play()` **之前** `registerPlugin(A)`，观察 A 的 `ready()` 何时被调。
  2. 等首帧出现（内核就绪）后，再 `registerPlugin(B)`，观察 B。
  3. 让某个插件的 `ready()` 抛异常，看后续插件是否照常就绪。
- **预期**：
  1. 第 1 步：A 的 `ready()` **不会**在注册时被调；首帧后由统一广播触发一次。
  2. 第 2 步：B 的 `ready()` **立即**被补调一次（不等首帧、不等下一次广播）。
  3. 两者**各只调一次**，不因补调而重复。
  4. 第 3 步：抛异常的插件只产生一条 `[LV-5004]` 日志，**不影响其它插件的就绪**
     （与 `destroyAll` 同一隔离策略）。
- **验收方式**：单测（`test/player.test.ts` 三条：就绪前 / 就绪后两种时序 + 异常隔离）。
- **补充说明（内部实现，不构成接入方契约）**：「何时算就绪」的判断由 `Player` 持有；
  `kernelReady` / `runHooks` 均为内部成员、不在公开面上，且由 `verify/visibility.mjs` 兜底防止再漏出去。

### US-57 `utils/` 精简：文案表并入 `i18n`，能力对齐与重连规则并入 `Player`（0.7.0）
