...
> 本文 live-sdk 侧数据：**2026-09-16 实测**（当前工作树；`0.5.0` 发布后已含数轮重构。统计口径见 §1.0）
...
**统计口径**（`2026-09-16` 实测，**当前工作树**；`0.5.0` 发布后已含数轮重构）：
...
| | live-sdk（当前工作树） | live-sdk `0.5.0`（已发布） | xgplayer `3.0.26`（核心包） | 比值 |
|---|---|---|---|---|
| unpacked 体积 | 849 kB | 831 kB | 2.23 MB | ~2.6× |
| 文件数 | 83 | 81 | 293 | ~3.5× |
